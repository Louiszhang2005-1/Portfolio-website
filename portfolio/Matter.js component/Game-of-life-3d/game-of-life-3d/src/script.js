class App {
  init() {
    ...
    this.geometry = new THREE.SphereBufferGeometry(this.meshSize, 32, 32);
    ...
    gridSize.add(this.gridGUI, 'rows', 10, 25)
      .onChange((size) => {
        this.pause = true;
        this.clearScene();
  ...
  draw() {
    ...
    for (let col = 0; col < this.grid.cols; col++) {
      for (let row = 0; row < this.grid.rows; row++) {
        const mesh = this.getMesh(this.geometry);
  ...
  updateGrid() {
    ...
    for (let col = 0; col < this.grid.cols; col++) {
      for (let row = 0; row < this.grid.rows; row++) {
        const cell = this.grid.data[col][row];
        cell.state = this.nextGrid[col][row].state;

        this.grid.handleState(cell);
      }
    }
  }
